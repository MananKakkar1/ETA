...
const generateThread = async (uid) => {
	try {
		const response = await fetch(`${API_BASE_URL}generate-thread`, {
			...
			body: JSON.stringify({ PRIMARY_KEY: uid }),
		});
		if (!response.ok) {
...
const sendMessage = async (uid, threadId, message) => {
	try {
		const response = await fetch(`${API_BASE_URL}send-message`, {
			...
			body: JSON.stringify({ PRIMARY_KEY: uid, threadId, message }),
		});
		if (!response.ok) {
...
const getThread = async (uid, threadID) => {
	try {
		const response = await fetch(`${API_BASE_URL}thread`, {
			...
			body: JSON.stringify({ PRIMARY_KEY: uid, chatID: threadID }),
		});
		if (!response.ok) {
...
function MessageBubble({ role, content, onClick }) {
	...
	return (
		<div
			className={`chat__bubble${isAgent ? ' chat__bubble--agent' : ''}${
