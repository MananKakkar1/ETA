  ...
  useEffect(() => {
    ...
    gltfLoader.load(
      ...
      (gltf) => {
        ...
        model.lookAt(
          new THREE.Vector3(camera.position.x, newCenter.y, camera.position.z)
        );
        model.rotation.x = 0;
        model.rotation.z = 0;
        ...
        const toCamera = new THREE.Vector3()
          .subVectors(camera.position, model.position)
          .normalize();
    ...
    fbxLoader.load(
      ...
      (err) => {
        console.warn(
          "No FBX animation loaded or failed to load /animation/Idle.fbx",
          err
        );
      }
    );
      ...
      <div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
        ...
        >
          ...
          <div style={{ marginTop: 8, fontSize: 14 }}>
            Virtual Teaching Assistant (Avatar.glb) — animation from
            /animation/Idle.fbx (if present)
          </div>
        </div>
        ...
        <div style={{ flex: 1, maxWidth: 600 }}>
          ...
          >
            <div style={{ flex: 1, overflowY: "auto", marginBottom: 8 }}>
              {messages.length === 0 && (
                <div style={{ color: "#666" }}>
                  No messages yet. Say hello to the Virtual TA.
                </div>
              )}
              {messages.map((m) => (
